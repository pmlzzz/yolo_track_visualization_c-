...
<body bgcolor="#272822">
<div id="videodiv">
<video id="video" width="900" opacity="1" src="./test.webm" >
...
<div style="color:#9F9F9F" id="button">
...
<!-- extract tracking data-->
  <?php
$sql = "SELECT blobs,points,frames,x,y,velocity,orientationx,orientationy from newtrack768x576m1 ";
...
while($rows = mysqli_fetch_assoc($result))
  ...
  <script>
    ...
    track.velocity.push(<?php echo($rows["velocity"]); ?>*10);
  ...
  <?php
         ...
         ?>
  <?php
$sql = "SELECT blobs,points,frames,x,y,velocity,orientationx,orientationy from newtrack768x576m1 order by frames ";
$result = $link->query($sql);if ($result->num_rows <= 0) {
       echo "0 results";
     }   
while($rows = mysqli_fetch_assoc($result))
  ...
  <script>
    ...
    point.velocity.push(<?php echo($rows["velocity"]); ?>*10);
...
for (var i = 0; i < point.frames.length; i++) {//4725
  ...
  var pp = {
  blobs:track.blobs[i],
  points:track.points[i],
  frames:track.frames[i],
  x:track.x[i],
  y:track.y[i],
  velocity:track.velocity[i],
  orientationx:track.orientationx[i],
  orientationy:track.orientationy[i]
  };
...
var svg = d3.select("#velocity").append("svg")
    .attr("width", (width + margin.left + margin.right)*2+200)
...
var tmpdata=framedata;
for (var i = 0; i <= framedata.length - 1; i++) {
  var sum=0,k=5;
  for (var j = i-k; j <= i+k; j++) {
    if(j<0||j>framedata.length - 1) sum+=0;
    else sum+=tmpdata[j]
  }
  framedata[i]=sum/(2*k+1);
}
...
var div = d3.select("body").append("div") 
  ...
  var arcx = d3.scale.linear()
    .domain([0, linexScale(50)])
    .range([0,framedata.length]);
 ...
 var brushg = svg.append("g")
  ...
  svg.append("text")
      ...
      .text("velocity timeline");
var offsetX=400;
  dragLine = svg.append('path')
  .attr('class', 'drag-line')
  .attr('d', `M${0},${y(1.5)}L${0},0`)
  ...
  .on("mouseover", function(d,i) {    
            div.transition()    
                .duration(200)    
                .style("opacity", .9);
            var X= parseInt(arcx(d3.event.pageX-57))
            div .html((X) + "<br/>"  + Math.floor((framedata[X]*100))/100)  
...
for (var i = 0; i <= a; i++) {
  point[i]=[];
  for (var j = 0; j <= b; j++) {
    point[i][j]={x:1,y:1,v:0,dx:0,dy:0};
  };
};
...
for (var i = track.blobs.length-1; i >0; i--) {
  ...
  point[a][b].dx=track.orientationx[i];
  point[a][b].dy=track.orientationx[i];
...
function RGBL(x,y)
{
  ...
  a1=0.6*V*(1-f)+0.4*V;
  b1=0.6*V*(1-(1-f))+0.4*V;
  min=0.4*V;
  switch(j)
  {
    case 0: R = V; G = b1; B = min;break;      
    case 1: R = a1; G = V; B = min;break;      
    case 2: R = min; G = V; B = b1;break;      
    case 3: R = min; G = a1; B = V;break;      
    case 4: R = b1; G = min; B = V;break;      
    case 5: R = V; G = min; B = a1;break;
  }
  R=parseInt(R);
  G=parseInt(G);
  B=parseInt(B);
...
var directionpoints=[];
for (var i = 0; i <= allpointss.length - 1; i++) {
  directionpoints.push(allpointss[i])
}
...
var rScale = d3.scale.linear().domain([0, 25]).range([0, 1]);

direction.attr({cx: function(d){ return d.orientationx*R(d.frames*1)+center.x;}, 
                 cy: function(d){return -d.orientationy*R(d.frames*1)+center.y;}, 
                 r:function(d){return 3+rScale(d.velocity)}, 
                 opacity: 0.5, 
                 fill:function(d){
                  return RGB(d.orientationx,d.orientationy);
                }
                });
var dlines=mainsvg.selectAll(".dlines")
.data(allpointss).enter().append("g").attr("class","dlines")
dlines
.append('line')
.attr("id",function (d,i) {return "line"+d.blobs;})
.attr({
  x1: function(d){return d.orientationx*R(d.frames*1)+center.x;}, 
  y1: function(d){return -d.orientationy*R(d.frames*1)+center.y;},
  x2: function(d,i){
    
    if (i>=allpointss.length-1) {return d.orientationx*R(d.frames*1)+center.x;} else{return allpointss[i+1].orientationx*R(allpoints[i+1].frames*1)+center.x;};
    
  },
  y2: function(d,i){
    if (i>=allpointss.length-1) {return d.orientationy*R(d.frames*1)+center.y;} else{return allpointss[i+1].orientationy*R(allpoints[i+1].frames*1)+center.y;};
...
lines
  .attr({
    x1: function(d){return -30*3+linexScale(d.frames)}, 
    y1: function(d){
    ...
    x2: function(d){return -30*3+linexScale(d.frames+1)},
    y2: function(d){
  ...
  .attr("opacity","1") 
  .attr("stroke",function(d){return RGBL(d.orientationx,d.orientationy);})
  .on("mouseover", function(d,i) {    
            linediv.transition()    
                .duration(200)    
                .style("opacity", .9);
            var X= parseInt((d3.event.pageX-57)/890*framenum)
            linediv.html(d.frames + "<br/>" +Math.floor((d.velocity*100))/100+ "<br/>" +d.blobs)  
                .style("left", (d3.event.pageX) + "px")   
                .style("top", (d3.event.pageY - 28) + "px");  
            videodiv.transition()    
                .duration(200)    
            ...
            d3.selectAll(".dlines").select("line[id=line"+d.blobs+"]").attr("stroke-width",0.5);
            })
  .on("mouseout", function(d,i) {
    d3.selectAll(".dlines").select("line[id=line"+d.blobs+"]").attr("stroke-width",0);
            });
